...
def session():
    """Authenticated session — logs in as admin and attaches Bearer token.
    Retries once after 65s if rate-limited, so tests remain stable when run
    alongside the auth rate-limit tests.
    """
    import time
    s = requests.Session()
